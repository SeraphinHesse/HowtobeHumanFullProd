...
- Seven domains exist: `balancing/{buildings,enemies,map,ui,core,vfx,
  ...
  **VfxAuthoringPLAN VA-1/VA-2** made three changes here. (1) The `vfx`
  category's `Effects` group is no longer a FLAT `slots` list — it now holds
  one leaf CHILD group per effect (`Hit`, `Muzzle`, `Crater`, …), the shape
  `walls`/`conditions`/`ui` already use. Zero schema change (`group_node`
  already recurses); it is what makes `selection.variant_target()` resolve and
  `registry_ops.add_variant` reachable, i.e. what turns "+ Variant" from
  structurally dead into live for VFX — the exact failure mode the
  variant-families note below warns a flat leaf group causes. (2)
  `trigger_row.sprite_slot`'s enum is now GENERATED by
  `tools/gen_sprite_slot_enum.py` from the live registry (it was hand-typed
  and had drifted to six keys against thirteen real slots), pinned by
  `test_schema_slot_sync.py`; its sibling `procedural` stays hand-maintained
  because those values name game-code kinds, not `procedural.*` keys. (3)
  Every `trigger_row` gained two required keys: `variant_select`
  (`{mode: random|level|misc, misc_key}` — how to choose among a slot's
  variants at play time) and `draw_in_front` (bool — a same-tile depth
  TIE-BREAK, not a layer switch). All ten rows ship `mode: "random"`,
...
- **`slots.json`'s `core` category gained a `"Moving Sign"` one-slot group**
  ...
  `camera_startpoint`, `camera_limit_centerpoint`, `start_area`,
  `tutorial_flute`, `tutorial_stone`) at
...
- **`maps/<id>.json` (map files)**: `id` (== filename stem, loader-enforced),
  ...
  `base_hole`), `camera_start` (nullable {col,row,slot}),
  `camera_limit_center` (nullable {col,row,slot}, slot const-pinned to
  `camera_limit_centerpoint` — `camera_start`'s single-tile twin, and the
  CENTRE of the camera's play area: `core` balancing's
  `Camera.max_offset_tiles_x/_y` travel limit is measured from it, the camera
  never starts on it, and like `start_area`/`tutorial_*` it is never emitted by
  a render emitter — the editor draws a blue outline. Null falls back to
  `camera_start`, then the map centre; existing maps were migrated to
  `"camera_limit_center": null`), `start_area`

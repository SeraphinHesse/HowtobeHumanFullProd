...
|-------|-------|---------------|--------|
| FP-1 | Freeze the JSON fixture snapshot + access point | `tools/tests/fixtures/data/**` (new), `tools/tests/fixture_data.py` (new) | not started |
| FP-2 | Flip the 47 live loads to `FIXTURE_DATA`; classify allowlist | ~42 files in `tools/tests/` | not started |
| FP-3 | Derive data-mirroring literals in the worst files | `test_levelup`, `test_lightning`, `test_enemies`, `test_tile_unlock`, `test_tile_conditions`, `test_boss`, `test_combat_speed`, `test_phase_loop` (± discoveries) | not started |

...
def _update_defender(defender, scene, enemies, dt, min_atk, proj_speed):
    ...
    # 10I: targeting uses the EFFECTIVE range (mountain +1); the raw
    # range_tiles() keeps feeding pathfinding coverage + the RANGE overlay.
    # Guarded so bare-bones defender stubs in tests keep working.
    rng = getattr(defender, "effective_range_tiles", defender.range_tiles)()
...
def _update_beam(defender, enemies, dt):
    ...
    center = (defender.col, defender.row)
    # 10I: effective (mountain-boosted) range for targeting, guarded for stubs.
    rng = getattr(defender, "effective_range_tiles", defender.range_tiles)()
    in_range = [e for e in enemies if _chebyshev(center, e) <= rng]

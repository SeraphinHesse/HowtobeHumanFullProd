...
class ScreenSkinning:
    ...
    def state_of(self, widget) -> str:
        ...
        PHASE (UL-4) always returns ``"idle"`` — a placeholder. UL-5 replaces
        ONLY this method's BODY with real per-widget state resolution
        (hover/pressed/disabled). Every call site passes this method BY
        REFERENCE (``self.skinning.state_of``, a bound method), never a
        hardcoded ``"idle"`` literal, so UL-5's diff is a single-method-body
        change, not 14 call-site edits."""
        del widget  # placeholder until UL-5
        return "idle"

...
Why this test is load-bearing. Once the gate SELECTS on markers
(`-m "not migration"` in CI), a module that carries no tier simply never runs.
It does not fail, it does not warn — it vanishes, and its coverage with it.

That is precisely the bug TG-2 just killed: test_balancing_parity silently
...
VALID_TIERS = {"core", "editor", "meta", "migration"}
...
class TestTierCoverage(unittest.TestCase):
    ...
    def test_every_tier_is_one_of_the_four(self):

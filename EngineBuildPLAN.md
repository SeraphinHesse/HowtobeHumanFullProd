...
| 9+ | `engine/physics` (E-30..E-32) + `Movement`/`RangeSensor` components first, then port gameplay domain-by-domain (map → buildings → enemies → core phases → ui), prototype repo as spec | each domain: acceptance checklist → test → implement → live playtest | in progress — 9B–9H (first playable) + 10A–10I landed; per-phase detail in MIGRATION_PLAN.md. **10G–10I (Boss / Lightning+cheats / Map depth)** landed as one parallel-subagent batch (briefs + rulings in `docs/briefs/`, workflow skill `.claude/commands/execute-plan-phases.md`): 10G boss rounds (era stats, `[boss]+siege+shuffle` queue, death swarm, shake, HP bars, announcement, BOSS_CUTSCENE A/B + boss_bonuses in payday slot 3, history popup); 10H lightning (level-1 start, projected-pixel-circle radius, ENEMY-dt cooldown, base-panel upgrade, HUD readout) + Ctrl+L cheat menu (`return_phase` levelup path live); 10I map depth (seeded tile conditions + all stat modifiers, damage-based weight reduction, defence-range coverage incl. boosts, RANGE/HEATMAP overlays in `game/ui/overlays.py`, terrain badges/tooltips). `py -m unittest discover` 815 tests, 94 new (`test_boss.py` 23 / `test_lightning.py` 31 / `test_tile_conditions.py` 41 — 1 shared fixture case), failures = the same pre-existing baseline set; `py tools/smoke.py` OK. **10E (Blocker + WallBuilder + edge walls)**: `game/buildings/structure.py` (StructureBuilding + Blocker/WallBuilder) + `WallBuilderState`; `TileMap` wall-edge registry (`WallEdge`/`place`/`remove`/`rebuild`/`damage_wall`) backing the live `get_wall_between`; enemy wall-attack via the existing PathAgent/EnemyCombat block-and-attack; payday slots 8 (teardown) + 10 (rebuild) filled. `py -m unittest discover` 703 tests (15 new in `test_structure.py`; 17 pre-existing failures = editor Qt/asset + deliberate parity drift + the known build flake — unchanged by 10E); `py tools/smoke.py` OK (19 data files); live windowed run confirmed |
